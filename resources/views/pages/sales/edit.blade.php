...
@section('content')
    <div class="container-fluid">
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <h1 class="h3 mb-0 text-gray-800 menu-title">Data Sales {{ $item->nama }}</h1>
        </div>
        @if ($errors->any())
                    ...
                    <div class="card show">
                        <div class="card-body">
                            <form action="{{ route('sales.update', $item->id )}}" method="POST">
                                ...
                                <div class="form-group row">
                                    <label for="cabang" class="col-1 col-form-label text-bold">Cabang</label>
                                    <span class="col-form-label text-bold">:</span>
                                    <div class="col-2">
                                        <select class="form-control form-control-sm" tabindex="5" id="cabang" name="cabang">
                                            <option value="1" @if($item->id_cabang == 1) selected @endif>Kantor</option>
                                            <option value="3" @if($item->id_cabang == 3) selected @endif>Cianjur</option>
                                        </select>
                                    </div>
                                </div>
                                <hr>
