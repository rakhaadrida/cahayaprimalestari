...
<div class="container-fluid">
  ...
  <div class="d-sm-flex align-items-center justify-content-between mb-0">
      <h1 class="h3 mb-0 text-gray-800 menu-title">Data Tanda Terima</h1>
  </div>
  @if ($errors->any())
            ...
            <form action="" method="">
              ...
              <div class="container so-container">  
                <div class="form-group row justify-content-center" >
                  ...
                  <div class="col-2">
                    <input type="text" class="form-control datepicker form-control-sm text-bold mt-1" name="tglAwal" id="tglAwal">
                  </div>
                  <label for="tanggal" class="col-auto col-form-label text-bold ">s/d</label>
                  <div class="col-2">
                    <input type="text" class="form-control datepicker form-control-sm text-bold mt-1" name="tglAkhir" id="tglAkhir">
                  </div>
              ...
              <table class="table table-sm table-bordered table-striped table-responsive-sm table-hover" id="dataTable" width="100%" cellspacing="0">
                <thead class="text-center text-bold text-dark">
                  ...
                  <th style="width: 70px" class="align-middle">No. Tanda Terima</th>
                  <th style="width: 80px" class="align-middle">Tgl Tanda Terima</th>
                  <th style="width: 70px" class="align-middle">No. Faktur</th>
                  <th style="width: 80px" class="align-middle">User</th>
                </thead>
                <tbody>
                  @php $i=1; @endphp
                  @forelse ($items as $item)
                    <tr class="text-dark">
                      ...
                      <td class="text-center"><button type="submit" formaction="{{ route('ttr-detail', $item->id) }}" formmethod="POST" class="btn btn-link btn-sm text-bold">{{ $item->id }}</button></td>
                      <td class="text-center align-middle">{{ \Carbon\Carbon::parse($item->tanggal)->format('d-M-y')  }}</td>
                      @php
                        $kode = \App\Models\TandaTerima::where('id', $item->id)->get();
                      @endphp
                      @if($kode->count() == 1)
                        <td class="text-center align-middle">{{ $kode[0]->id_so }}</td>
                      @else
                        <td class="text-center align-middle">{{ $kode[0]->id_so }} - {{ $kode->last()->id_so }}</td>
                      @endif
