                            ...
                            <form action="">
                                        ...
                                        @if(Auth::user()->roles == 'CIANJUR')
                                            ...
                                            <div class="col-2">
                                                <select class="form-control form-control-sm mt-1" tabindex="5" id="gudang" name="gudang">
                                                    <option value="0" @if($idGudang == 0) selected @endif>Supplier</option>
                                                    <option value="1" @if($idGudang == 1) selected @endif>Toko</option>
                                                </select>
                                            </div>
                                        @endif
                                ...
                                <div class="row justify-content-center" style="margin-bottom: 15px">
                                    <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                                        <button type="submit" formaction="{{ route('rs-pdf-filter') }}" formmethod="POST" formtarget="_blank" class="btn btn-primary btn-block text-bold">Download PDF</button>
                                    </div>
                                    @if(Auth::user()->roles != 'CIANJUR')
                                        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
                                            <button type="submit" formaction="{{ route('rs-excel-filter') }}" formmethod="POST" class="btn btn-danger btn-block text-bold">Download Excel</button>
                                        </div>
                                    @endif
                                </div>
                                <hr>
                                                        ...
                                                        @foreach($sub as $s)
                                                            ...
                                                            @foreach($barang as $b)
                                                                ...
                                                                <tr class="text-dark text-bold collapse show" id="collapseSub{{$s->id}}">
                                                                    ...
                                                                    @else
                                                                        <td class="text-right" style="background-color: yellow">{{ $stok->count() != 0 ? $stok[0]->total + $tambah->first()->qty + $tambahRet->first()->qty - $kurang->first()->qty - $kurangRet->first()->qty : '' }}</td>
                                                                        @foreach($gudang as $g)
                                                                            @php
                                                                                if($g->tipe != 'RETUR') {
                                                                                    ...
                                                                                    $detilRT = \App\Models\DetilRT::join('returterima', 'returterima.id',
                                                                                            ...
                                                                                            ->whereBetween('returbeli.tanggal', [$awal, $kemarin])
                                                                                            ->get();
                                                                                    $tambahGd = \App\Models\DetilRB::join('returbeli', 'returbeli.id',
                                                                                              'detilrb.id_retur')
                                                                                              ->selectRaw('sum(qty_retur) as retur')
                                                                                              ->where('status', 'INPUT')->where('id_barang', $b->id)
                                                                                              ->whereBetween('tanggal', [$awal, $kemarin])
                                                                                    ...
                                                                                    $kurangGd = \App\Models\DetilRJ::join('returjual', 'returjual.id',
                                                                                              'detilrj.id_retur')
                                                                                              ->selectRaw('sum(qty_retur - qty_kirim - potong) as qty')
                                                                                              ->where('status', 'INPUT')->where('id_barang', $b->id)
