...
for(let i = 0; i < hapusBaris.length; i++) {
  hapusBaris[i].addEventListener("click", function (e) {
    kurangAkhir.value = addCommas(+kurangAkhir.value.replace(/\./g, "") + +cicilDetil[i].value.replace(/\./g, ""));
    totalAkhir.value = addCommas(+totalAkhir.value.replace(/\./g, "") - +cicilDetil[i].value.replace(/\./g, ""));

    for(let j = i+1; j < hapusBaris.length; j++) {
      kurangDetil[j].value = addCommas(+kurangDetil[j].value.replace(/\./g, "") + +cicilDetil[i].value.replace(/\./g, ""));
    }

    if(kurang[0].value != '')
      kurang[0].value = addCommas(+kurang[0].value.replace(/\./g, "") + +cicilDetil[i].value.replace(/\./g, ""));
...
for(let i = 0; i < hapusBiasa.length; i++) {
  hapusBiasa[i].addEventListener("click", function (e) {
    kurangAkhir.value = addCommas(+kurangAkhir.value.replace(/\./g, "") + +cicilModal[i].value.replace(/\./g, ""));
    totalAkhir.value = addCommas(+totalAkhir.value.replace(/\./g, "") - +cicilModal[i].value.replace(/\./g, ""));
