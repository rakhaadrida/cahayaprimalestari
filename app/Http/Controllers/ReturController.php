...
class ReturController extends Controller
{
    ...
    public function createReturJual($id) {
        ...
        $barang = Barang::All();
        $stokBagus = StokBarang::where('id_gudang', 'GDG01')->where('status', 'T')->get();
    ...
    public function storeTerimaBeli(Request $request) {
        ...
        $kodeBarang = []; $kodeTerima = []; $statusCetak = 'false';
        if($items->count() != 0) {
            if($jum != $items->count()) {
                $k = 0;
                foreach($items as $i) {
...
//        $retur->save();
    ...
    public function selesaiReturBeli($id) {
        $waktu = Carbon::now('+07:00');
        $bulan = $waktu->format('m');
        $month = $waktu->month;
        $tahun = substr($waktu->year, -2);
